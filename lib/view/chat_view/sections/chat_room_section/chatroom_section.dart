...
class ChatroomSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var providerWatch = context.watch<ChatroomSectionProvider>();
    var providerRead = context.read<ChatroomSectionProvider>();
    return Expanded(
      child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: Builder(builder: (context) {
                  return ListView.builder(
                    // Logic part
                    itemCount: providerWatch.addMessage.length,
                    itemBuilder: (context, index) {
                      return MouseRegion(
                        onEnter:(_)=> providerRead.updateHoverStatus(index, true),
                        onExit: (_)=> providerRead.updateHoverStatus(index, false),
                        child: MessageBubble(
                          message: providerWatch.addMessage[index],
                          image: AssetImage(ImagePath.ProfileImage),
                          name: "Zeeshan",
                          date: providerWatch.ishover[index] 
                              ? "Yesterday"
                              : "",
                          time: providerWatch.ishover[index]
                              ? "9:23 AM"
                              : "",
                          custom_action:
                              providerWatch.ishover[index]
              ...
              Padding(
                ...
                child: TextField(
                  // Logic part
                  onChanged: (value) => providerRead.notifyListeners(),
                  cursorColor: AppColor.grey,
                  onSubmitted: (_)=>providerRead.addNewMessage(),
                  controller: providerWatch.MessageController,
                          ...
                          child: Center(
                            child: Row(
                              ...
                              children: [
                                SizedBox(width: 7),
                                InkWell(
                                  onTap: ()=>providerRead.addNewMessage(),
                                  child: ChatroomCustomIcons(
                                    Color: providerWatch.hasText()
                                        ? AppColor.royalBlue
